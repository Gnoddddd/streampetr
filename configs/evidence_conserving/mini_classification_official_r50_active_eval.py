...
data_root = '/home/research/research/evidence3d/data/nuscenes-mini/'
input_modality = dict(
    use_lidar=False,
    use_camera=True,
    use_radar=False,
...
train_pipeline = [
    ...
    dict(
        ...
        collect_keys=[
            ...
            'img_timestamp', 'ego_pose', 'ego_pose_inv', 'camera_online_mask',
            'camera_quality', 'camera_fresh_mask', 'corruption_severity',
            'prev_exists'
        ]),
    dict(
        type='Collect3D',
        keys=[
            ...
            'ego_pose_inv', 'camera_online_mask', 'camera_quality',
        ...
        transforms=[
            dict(
                ...
                collect_keys=[
                    ...
                    'camera_online_mask', 'camera_quality',
                    'camera_fresh_mask', 'corruption_severity'
                ],
                class_names=[
                    'car', 'truck', 'construction_vehicle', 'bus', 'trailer',
...
data = dict(
    ...
    workers_per_gpu=2,
    train=dict(
        type='EvidenceNuScenesDataset',
        data_root='/home/research/research/evidence3d/data/nuscenes-mini/',
        ann_file=
        '/home/research/research/evidence3d/data/nuscenes-mini/nuscenes2d_temporal_infos_train.pkl',
        pipeline=[
            dict(type='LoadMultiViewImageFromFiles', to_float32=True),
            dict(
                type='ApplyPartialObservation',
            ...
            dict(
                ...
                collect_keys=[
                    ...
                    'img_timestamp', 'ego_pose', 'ego_pose_inv',
                    'camera_online_mask', 'camera_quality',
                    'camera_fresh_mask', 'corruption_severity', 'prev_exists'
                ]),
            dict(
                type='Collect3D',
                keys=[
                ...
                meta_keys=('filename', 'ori_shape', 'img_shape', 'pad_shape',
                           'scale_factor', 'flip', 'box_mode_3d',
                           'box_type_3d', 'img_norm_cfg', 'scene_token',
                           'sample_idx', 'frame_idx', 'gt_bboxes_3d',
                           'gt_labels_3d'))
        ],
        classes=[
            'car', 'truck', 'construction_vehicle', 'bus', 'trailer',
    ...
    val=dict(
        ...
        data_root='/home/research/research/evidence3d/data/nuscenes-mini/',
        ann_file=
        '/home/research/research/evidence3d/data/nuscenes-mini/nuscenes2d_temporal_infos_val.pkl',
        pipeline=[
            dict(type='LoadMultiViewImageFromFiles', to_float32=True),
            dict(
                type='ApplyPartialObservation',
                ...
                transforms=[
                    dict(
                        ...
                        collect_keys=[
                            ...
                            'ego_pose_inv', 'camera_online_mask',
                            'camera_quality', 'camera_fresh_mask',
                            'corruption_severity'
                        ],
                        class_names=[
...
checkpoint_config = dict(interval=10, by_epoch=False, max_keep_ckpts=1)
log_config = dict(
    interval=1, hooks=[dict(type='TextLoggerHook', by_epoch=False)])
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = 'outputs/eval_classification_official_r50_active'
load_from = None
resume_from = None
workflow = [('train', 1)]
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        temporal_update_cfg=dict(
            gamma=0.9, evidence_scale=2.0, max_effective_count=6.0),
        policy_cfg=dict(
            keep_observability=0.45,
            keep_presence=0.28,
            keep_max_uncertainty=0.55,
            recover_presence=0.3,
            recover_max_uncertainty=0.8,
            recover_max_age=3,
            recover_min_prior_strength=0.5,
            strong_negative=0.6,
            recover_score_scale=0.75,
            defer_score_scale=0.2),
...
PROJECT_ROOT = '/home/research/research/evidence3d'
DATA_ROOT = '/home/research/research/evidence3d/data/nuscenes-mini'
PROTOCOL_FILE = os.environ.get('EVIDENCE3D_PROTOCOL', None)
custom_imports = dict(
    imports=['evidence3d_plugin'], allow_failed_imports=False)
custom_hooks = [dict(type='EvidenceTraceHook', interval=20)]
seed = 2026

# EVIDENCE3D_BIND_EVAL_PROTOCOL_BEGIN
# 仅将外部协议绑定到实际测试数据管线。
# 不修改训练管线，避免影响已有训练配置和权重。
def _evidence3d_bind_eval_protocol(node):
    if isinstance(node, dict):
        if node.get("type") == "ApplyPartialObservation":
            node["training"] = False
            node["schedule_file"] = PROTOCOL_FILE

        for child in node.values():
            _evidence3d_bind_eval_protocol(child)
...
_evidence3d_bind_eval_protocol(data.get("test"))
del _evidence3d_bind_eval_protocol
# EVIDENCE3D_BIND_EVAL_PROTOCOL_END
