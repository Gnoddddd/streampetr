...
def require_ready() -> tuple[dict, dict]:
    ...
    if not validation_path.exists() or not progress_path.exists():
        raise RuntimeError("run scripts/prepare_care3d_p1.py first")
    validation = json.loads(validation_path.read_text())
    progress = json.loads(progress_path.read_text())
    if validation.get("status") != "VALIDATED_BEFORE_P1_FORWARD":
        raise RuntimeError("P1 validation not frozen")
    ...
    for split, expected in (("probe_train", 419), ("probe_val", 133)):
        value = supervision.get(split, {})
        if int(value.get("completed_scenes", -1)) != expected:
    ...
    if test_dir.exists() and any(test_dir.glob("*.complete.json")):
        raise RuntimeError("probe-test evaluation exists before P1 training freeze")
...
def flatten_protocols(arrays: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    for protocol_index in range(len(PROTOCOLS)):
        ...
        output["fault_topk_threshold"].append(
            arrays["fault_topk_threshold"][mask, protocol_index]
        )
        output["cross_topk"].append(arrays["cross_topk"][mask, protocol_index])
        output["protocol_index"].append(
            np.full(int(mask.sum()), protocol_index, dtype=np.int64)
        )
...
class P1Dataset(Dataset):
    def __init__(self, arrays: dict[str, np.ndarray]) -> None:
        ...
        if len(sizes) != 1:
            raise RuntimeError(f"P1 flattened arrays are misaligned: {sizes}")
...
def to_device(batch, device):
    output = {}
    integer = {"target_class", "protocol_index"}
...
def batch_loss(batch, p0, p1, classifier, loss_cfg):
    with torch.no_grad():
        p0_output = p0(
            ...
            batch["decision_features"],
            batch["camera_support"],
            batch["camera_quality"],
        )
        ...
        if not torch.allclose(
            computed_fault_score, batch["fault_score"], rtol=0.0, atol=5e-4
        ):
    ...
    routed, aux = p1(
        ...
        p0_output["vulnerability"].detach(),
        p0_output["boundary_crossing_logits"].detach(),
        batch["protocol_index"],
        fault_active=True,
    )
    routed_logits = classifier(routed)
    losses = p1_score_routing_loss(
        routed_query=routed,
        clean_query=batch["clean_query"],
        fault_query=batch["fault_query"],
        routed_logits=routed_logits,
        fault_logits=fault_logits.detach(),
        target_class=batch["target_class"],
        clean_score=batch["clean_score"],
        fault_score=batch["fault_score"],
        fault_topk_threshold=batch["fault_topk_threshold"],
        cross_topk=batch["cross_topk"],
        **loss_cfg,
    )
...
def update_progress() -> None:
    ...
    completed = []
    for seed in SEEDS:
        manifest = REPORT / "training" / f"seed_{seed}" / "training_manifest.json"
        if not manifest.exists():
            continue
        value = json.loads(manifest.read_text())
        if value.get("status") == "P1_TRAINING_COMPLETE_TEST_UNSEEN" \
...
def main() -> None:
    ...
    train_dataset = P1Dataset(train)
    val_dataset = P1Dataset(val)
    ...
    p1 = CARE3DP1ScoreRouter(
        object_dim=int(router_cfg["object_dim"]),
        source_dim=int(router_cfg["source_dim"]),
        vulnerability_dim=int(router_cfg["vulnerability_dim"]),
    ...
    p1_parameter_ids = {id(parameter) for parameter in p1.parameters() if parameter.requires_grad}
    ...
    if any(parameter.requires_grad for parameter in p0.parameters()):
        raise RuntimeError("frozen P0 has trainable parameters")
    if any(parameter.requires_grad for parameter in classifier.parameters()):
        raise RuntimeError("frozen detector classifier has trainable parameters")
    ...
    manifest = {
        ...
        "router_parameters_in_optimizer": sum(parameter.numel() for parameter in p1.parameters()),
