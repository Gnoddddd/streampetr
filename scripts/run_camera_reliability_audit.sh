...
config="configs/stage4/camera_reliability_b0_audit.py"
run_root="$root/outputs/stage4/camera_reliability_audit"
overall_status=0
...
run_one() {
  ...
  local -a command=(
    ...
    --split val
    --camera-attention-dir "$target/trace"
...
run_one crash_left_pair protocols/presets/multi_camera_left_10f.json

if [[ "$overall_status" -eq 0 ]]; then
  python scripts/analyze_camera_reliability.py
  overall_status=$?
fi
exit "$overall_status"
